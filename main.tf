...
resource "aws_instance" "myways_instance" { 
    ...
    tags = {
      Name = "My_ways_instance" # Tags the EC2 instance with a name for easier identification, here 'My_ways_instance'
    }
}

# Creation of s3 bucket
resource "aws_s3_bucket" "myways_s3_bucket" { 
  bucket = "myways-s3-bucket-2184" # Defines the S3 bucket name to store data and documents. The name must be unique globally, here 'myways-s3-bucket-2184'
}
